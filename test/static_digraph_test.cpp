...
GTEST_TEST(StaticDigraph, vectors_constructor) {
    ...
    for(auto u : graph.nodes())
        EXPECT_TRUE(graph.is_valid_node(u));
    EXPECT_FALSE(graph.is_valid_node(8));

    for(auto a : graph.arcs())
        EXPECT_TRUE(graph.is_valid_arc(a));
    EXPECT_FALSE(graph.is_valid_arc(9));    

    AssertRangesAreEqual(graph.out_neighbors(0), std::ranges::empty_view<StaticDigraph::Node>());
    AssertRangesAreEqual(graph.out_neighbors(1), std::vector<StaticDigraph::Node>({2,6,7}));
    AssertRangesAreEqual(graph.out_neighbors(2), std::vector<StaticDigraph::Node>({3,4}));
    AssertRangesAreEqual(graph.out_neighbors(6), std::vector<StaticDigraph::Node>({5}));
    AssertRangesAreEqual(graph.out_neighbors(7), std::ranges::empty_view<StaticDigraph::Node>());
