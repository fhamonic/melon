...
// vertices. reset() reaches the same read through _reset_current_vertex().
...
// Without the constructor's advance() redone, the queue stays empty while
...
// An unconstrained `connected_components(UG &&)` beats the copy constructor
// for a non-const lvalue and tries to build the algorithm out of itself;
...
// The adjacency concepts are independent of them, so a constraint spelled
// with those gates the wrong thing in both directions: a graph with
// out_neighbors/in_neighbors but no out_arcs/in_arcs passes it and then
// hard-errors inside undirect.
...
// The call fails as an unsatisfied constraint naming
// weakly_connected_components, not as a hard error deep inside
// views::undirect.
