...
////////////////////////////////////////////////////////////////////////////////
// bellman_ford_moore computes shortest distances from the sources by rounds
// that rescan only the vertices the previous round improved; run() drains it
// in one call, and like bellman_ford it accepts negative lengths
////////////////////////////////////////////////////////////////////////////////
...
GTEST_TEST(bellman_ford_moore, test) {
    ...
    std::cout << "bellman_ford_moore size: " << sizeof(decltype(alg))
              << std::endl;
...
struct bellman_ford_moore_detect_traits {
    using semiring = shortest_path_semiring<int>;
    static constexpr bool store_paths = false;
    static constexpr bool detect_negative_cycles = true;
};

struct bellman_ford_moore_full_traits {
    using semiring = shortest_path_semiring<int>;
    static constexpr bool store_paths = true;
    static constexpr bool detect_negative_cycles = true;
};
...
GTEST_TEST(bellman_ford_moore, path_to) {
    ...
    auto alg =
        bellman_ford_moore(bellman_ford_moore_path_traits{}, graph, length_map);
...
GTEST_TEST(bellman_ford_moore, negative_lengths) {
...
GTEST_TEST(bellman_ford_moore, negative_cycle_retrieval) {
    static_digraph_builder<static_digraph, int, int> builder(4);
    ...
    auto alg =
        bellman_ford_moore(bellman_ford_moore_full_traits{}, graph, length_map);
    alg.add_source(0u).run();
    ASSERT_TRUE(alg.found_negative_cycle());
    ...
    ASSERT_TRUE(EQ_MULTISETS(
        ...
        {1, 2}));
    // in path order: each arc's target is the next one's source
...
////////////////////////////////////////////////////////////////////////////////
// the certifying round is not fooled by a graph that legitimately needs all
// n-1 computing rounds: a busy queue after the last of them is not a cycle
// until the certifying round confirms it
////////////////////////////////////////////////////////////////////////////////

GTEST_TEST(bellman_ford_moore, needing_every_round_is_not_a_cycle) {
    // chain 3 -> 2 -> 1 -> 0: each round improves exactly one more vertex,
...
////////////////////////////////////////////////////////////////////////////////
// vertices unreachable from the sources never enter the queue and stay at
// infty
////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////
// the queue needs out_arcs: unlike bellman_ford, an arc-list-only structure
// is rejected at the class head -- this is the trade against the arc sweep
////////////////////////////////////////////////////////////////////////////////

namespace incidence_required {
template <typename GG, typename MM>
concept spellable = requires { typename bellman_ford_moore<GG, MM>; };

using ALG = views::graph_all_t<arc_list_digraph &>;
using ALM = maps::mapping_all_t<static_map<unsigned int, int> &>;
}  // namespace incidence_required

static_assert(melon::graph<arc_list_digraph> &&
              !melon::outward_incidence_graph<arc_list_digraph>);
static_assert(!incidence_required::spellable<incidence_required::ALG,
                                             incidence_required::ALM>);
...
static_assert(!opt_in::reports_negative_cycles<
              bellman_ford_moore<opt_in::G, opt_in::LM>>);
static_assert(opt_in::reports_negative_cycles<bellman_ford_moore<
                  opt_in::G, opt_in::LM, bellman_ford_moore_detect_traits>>);
// retrieval needs the pred maps on top of detection
static_assert(!opt_in::retrieves_negative_cycles<bellman_ford_moore<
                  opt_in::G, opt_in::LM, bellman_ford_moore_detect_traits>>);
static_assert(!opt_in::retrieves_negative_cycles<bellman_ford_moore<
                  opt_in::G, opt_in::LM, bellman_ford_moore_path_traits>>);
static_assert(opt_in::retrieves_negative_cycles<bellman_ford_moore<
                  opt_in::G, opt_in::LM, bellman_ford_moore_full_traits>>);
...
// run(), add_source() and reset() push into the queue -- which allocates --
// and execute the user's length map and semiring; an unconditional noexcept
// would turn any throw from those into std::terminate.
namespace {
using probe_bellman_ford_moore = bellman_ford_moore<
    graph_ref_view<static_digraph>,
    mapping_ref_view<const static_map<unsigned int, int>>,
    bellman_ford_moore_default_traits<graph_ref_view<static_digraph>, int>>;
}  // namespace

static_assert(!noexcept(std::declval<probe_bellman_ford_moore &>().run()));
static_assert(!noexcept(std::declval<probe_bellman_ford_moore &>().reset()));
static_assert(!noexcept(std::declval<probe_bellman_ford_moore &>().add_source(
...
// written-out `bellman_ford_moore<G, LM>` did not compile, and there would be
// no way to name the type CTAD had just produced. The default is on the class
...
using written_out = bellman_ford_moore<G, LM>;

template <typename... Ts>
using deduced = decltype(bellman_ford_moore(std::declval<Ts>()...));
}  // namespace traits_default
...
static_assert(
    std::same_as<traits_default::written_out,
                 bellman_ford_moore<traits_default::G, traits_default::LM,
                                    bellman_ford_moore_default_traits<
...
GTEST_TEST(bellman_ford_moore, default_traits_spelling_runs) {
    ...
    bellman_ford_moore<views::graph_all_t<decltype(graph) &>,
