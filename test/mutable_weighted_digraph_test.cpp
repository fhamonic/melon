...
static_assert(melon::concepts::graph<mutable_weighted_digraph<int, int>>);
static_assert(
    melon::concepts::incidence_list_graph<mutable_weighted_digraph<int, int>>);
static_assert(
    melon::concepts::adjacency_list_graph<mutable_weighted_digraph<int, int>>);
static_assert(
    melon::concepts::has_vertex_map<mutable_weighted_digraph<int, int>>);

GTEST_TEST(mutable_weighted_digraph, empty_constructor) {
    using Graph = mutable_weighted_digraph<int, int>;
    Graph graph;
...
GTEST_TEST(mutable_weighted_digraph, create_vertices) {
    using Graph = mutable_weighted_digraph<int, double>;
    Graph graph;

    auto a = graph.create_vertex(0);
    auto b = graph.create_vertex(3);
    auto c = graph.create_vertex(7);
...
GTEST_TEST(mutable_weighted_digraph, create_arcs) {
    ...
    ASSERT_EQ(graph.nb_arcs(), 3);

    ASSERT_EQ(graph.target(ab), b);
    ASSERT_EQ(graph.target(ac), c);
    ASSERT_EQ(graph.target(cb), b);

    ASSERT_EQ(graph.arc_weight(ab), 0.2);
    ASSERT_EQ(graph.arc_weight(ac), 1.0);
    ASSERT_EQ(graph.arc_weight(cb), 3.14);
