...
GTEST_TEST(mutable_digraph, fuzzy_test) {
    ...
    for(std::size_t i = 0; i < 2000; ++i) {
        ...
        ASSERT_TRUE(EQ_MULTISETS(arcs_entries(graph), dummy_graph.arcs_entries()));
        ...
        for(auto && v : vertices(graph)) {
            ...
            ASSERT_TRUE(EQ_MULTISETS(in_arcs(graph,v), dummy_graph.in_arcs(v)));
            ASSERT_TRUE(
