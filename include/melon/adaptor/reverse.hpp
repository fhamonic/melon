...
public:
    ...
    auto nb_vertices() const
        requires requires(const G & g) { g.nb_vertices(); }
    {
        return _graph.get().nb_vertices();
    }
    auto nb_arcs() const noexcept
        requires requires(const G & g) { g.nb_arcs(); }
    {
    ...
    template <typename T>
        requires concepts::has_arc_map<G> decltype(auto)
